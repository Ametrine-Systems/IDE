...
import subprocess,os,platform,json,clipboard,asyncio,aiofiles
...
width=int(compiler.winfo_width()*0.111)
height=int(compiler.winfo_height()*0.056)
mainarea = Frame(compiler, bg='#CCC', width=width/7*4, height=height)
mainarea.pack(expand=True, fill='both', side='right')

sidebar = Frame(compiler, bg='white', relief='sunken', borderwidth=2, height=height,width=width*0.2)
...
current_file=Label(mainarea,text=file_path,bg="#fff",fg="#000",width=width)
current_file.pack()

editor = Text(mainarea,bg="#fff",fg="#000",insertbackground="#000",width=int(width*0.8),height=int(height/2))
editor.pack()
...
code_output = Text(mainarea,width=int(width*0.8),height=int(height/2.15))
code_output.pack()

home = ImageTk.PhotoImage(Image.open('assets\\home.png').resize((40,40),Image.ANTIALIAS))
settings = ImageTk.PhotoImage(Image.open('assets\\settings.png').resize((40,40),Image.ANTIALIAS))
brush = ImageTk.PhotoImage(Image.open('assets\\brush.png').resize((40,40),Image.ANTIALIAS))

frame = Frame(compiler,bg="black")
height=int(height/0.056)
...
def m_delete():
    global lastChoose,lastIndex
    if lastChoose==None:
        return messagebox.showerror("Error","Bitte wähle zuerst eine Datei!")
    response=messagebox.askyesno(title="Sicher?", message=f"Möchtest du {lastChoose} wirklich löschen?",icon='question',default='no')
    if response:
        to_delete=lastChoose
        for a in __file__.split("\\")[1:]:
            to_delete=to_delete.replace(a+"\\","")
        if to_delete.startswith("\\"):
            to_delete=to_delete[1:]
        os.remove(to_delete)
        current_file.configure(text="")
        editor.delete('1.0', END)
        set_file_path("")
        myListBox.delete(lastIndex)

def m_copy():
    global lastChoose
    if lastChoose==None:
        return messagebox.showerror("Error","Bitte wähle zuerst eine Datei!")
    clipboard.copy(lastChoose)

def m_paste():
    asyncio.run(m_paste2())
...
async def m_paste2():
    ...
    if not os.path.isfile(copied):
        return messagebox.showerror("Error","Nichts kopiert.")
    try:
        open(get_file_selector()+get_last_after_slash(copied),"w+")
        content=open(copied,"r").read()
    except Exception as e:
        messagebox.showerror("Error",e)
    else:
        async with aiofiles.open(get_file_selector()+get_last_after_slash(copied),"w") as f:
            f.write(content)
...
m.add_command(label="Delete",command=m_delete)
m.add_command(label="Copy",command=m_copy)
m.add_command(label="Paste",command=m_paste)
m.add_command(label="Reload",command=m_reload)
m.add_separator()
m.add_command(label="Rename",command=m_rename)
